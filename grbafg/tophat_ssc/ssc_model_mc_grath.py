...
# from ebltable.tau_from_model import
...
MAXDIMS = 10001
# @jit
def simpsint(N: int, dg: np.ndarray, di: np.ndarray, dx: np.ndarray, a: float, b: float):
...
def main1(z, gamma, theta, B, r, w_p_soll, Emin, Emax, Ebreak, p1, p2, ECFlag, Mbh, Mdot, blobDist, EBLModel):
    ...
    DL_Mpc = NedWright(z, source['h0'], source['Omega_M'], source['Omega_Lambda'], age_Gyr, zage_Gyr, DTT_Gyr, DA_Mpc,
    ...
    ple = fill_le(ple, source['E_min'], source['E_max'], source['E_break'],
                  source['lum'], source['pow1'], source['pow2'])
...
erg2TeV = 0.624151
from .models import Source, cgs
def main2():
    ss = Source()
    ebreak = max(2., max(ss.gm, ss.gc)) * cgs.mec2 / erg2TeV
    ebreak = max(2., max(ss.gm, ss.gc)) * cgs.mec2 / erg2TeV
...
def info2(source: dict, ple: dict, psync: dict, pic):
    w_sy = 0
    ...
    t_sync = 3. / 4. * (C_me_eV / (w_B * erg_eV)) / (SI_tcs * 10000. * SI_c * 100.) / (
                (10. ** source['E_break']) / C_me_eV)
...
# TODO For each gamma_e[i] compute Psynch
def synchrotron(ple: dict, psync: dict, B):
    x = ple['numbins']
...
# TODO For each inu in psync['numbins'], compute psync (for given gamma_e)
def sync_spec(psync: dict, energy: float, flux: float, B: float):
    ...
    for inu in range(0, psync['numbins']):
        nu = psync['e'][inu]  # Hz
        dalpha = np.pi / 24
        alpha = 0.0
        'All pitch angles'
        for j in range(0, 13):
            ...
            if (alpha > 0) and (alpha < np.pi - 0.0001):
                ...
                (Jp/Omega)
                """
                p = math.sqrt(3) / 8 / (np.pi ** 2) * (SI_e ** 3) * B * math.sin(
                    alpha) / SI_e0 / SI_me / SI_c * f * 0.5 * math.sin(alpha)
                'per freq'
                p *= (2 * np.pi) * J_eV * 2
                dg[j] = float(p)
        ...
        psync['j'][inu] += flux * simpsint(j, dg, di, dxx, 0.0, np.pi / 2) / 4 / np.pi
        _i = 1;
    return psync
...
def selfCompton(ple: dict, pseed, pic, r, idn):
...
def inv_spec(pseed: dict, pic: dict, energy: float, flux: float, r, ie, idn, flag):
    # initialize grids
    dxx, dg, di = [0.0] * MAX_BINS, [0.0] * MAX_BINS, [0.0] * MAX_BINS
...
@jit
def aver_jones(G_e: float, q2: float, q3: float, delta: float, dEs: float):
    """
    steps should be ~20 times smaller than the distance of the maximum
    in the scattered photon density in energy space from the maximum achievable energy
    ...
    while (q <= qmax):
        ...
        if (ifirst == 1):
            if (G_e < 1E+5):
                ...
                sums += weight * (2. * q * (math.log(q)) + (1. + 2. * q) * (1. - q) + 0.5 * ((G_e * q) ** 2) / (
                            1. + G_e * q) * (1. - q))
            else:
...
def F(x: float):
    ...
    if (x <= 0.001):
        res = (4. * np.pi / math.sqrt(3) / Gamma(1. / 3.) * ((x / 2.) ** (1. / 3.)) * (
...
def get_table(t_x: np.ndarray, t_v: np.ndarray, numbins: int, x: float):
    count = 0
...
def init_st(p1: float, p2: float, numbins: int) -> dict:
    i = 0
    p = {
        'x1': p1,
        'x2': p2,
...
def print_st(ple: dict, ichoice: int, source: dict, idn: int):
...
def spectrum(idn: int, source: dict, ple: dict, psync: dict, pic: dict, pbb: dict, pec: dict, psum: dict, data):
...
def calculateDisk(source, pbb, ple, pec, pic):
    ...
    for i in range(0, DISKBINS):
        ...
        for j in range(0, pbb['numbins']):
            ...
            """
            uw = 8 * np.pi * SI_h * ((pbb['e'][j] / SI_c) ** 3) / (np.exp(
                SI_h * pbb['e'][j] / (SI_k * T)) - 1.)  # Rybicki, Lightman "Radiative processes in Astrophysics" pg. 19
            ...
            """
            nDash = uNuDash / (SI_h * nuDash) * (
                        deltaJet * pbb['de'][j])  # 'd/dnuDash nDash ->  d/dnuDash nDash x Delta_nuDash'
            if (ifl):
            ...
            for k in range(0, ple['numbins']):
                ...
                for l in range(0, NUMPAVER):
                    ...
                    if (y < 1E-2):
                        ...
                        sigma = (SI_tcs * 3. / 4. * ((1 + y) / (y ** 3) * (
                                    2. * y * (1. + y) / (1. + 2. * y) - math.log(1. + 2. * y)) + 1. / 2. / y * math.log(
                            1. + 2. * y) - (1. + 3. * y) / ((1. + 2. * y) ** 2)))

                    'power emitted by electrons (d/dOmega d/dE N(E) * Delta E)'
                    p = (ple['f'][k] * ple['de'][k] * SI_c * (1. + muPhEl) * sigma * nDash / (NUMPAVER) * DeltaE / (
                                4. * np.pi))
            ...
            if (source['theta'] > 10. / 180. * np.pi):
                print(
                    "ATTENTION: gamma-ray absorption in the disk photon field is computed assuming that the jet angle is small (<10 degree)\n")
            for k in range(0, NUMABSORB):
                ...
                for l in range(0, 2):
                    ...
                    for m in range(0, p['numbins']):
                        nuThr = (2. * ((SI_me * SI_c * SI_c) ** 2) / (
                                    (SI_h ** 2) * (source['delta'] * p['e'][m]) * (1. + mu)))
                        if (nuThr > pbb['e'][j]):
                            continue
                        betaGG = np.sqrt(1. - nuThr / pbb['e'][j])
                        sigmaGG = (3. / 16. * SI_tcs * (1. - (betaGG ** 2)) * (
                                    2. * betaGG * ((betaGG ** 2) - 2.) + (3. - ((betaGG ** 2) ** 2)) * math.log(
                                (1. + betaGG) / (1. - betaGG))))
...
def ssa(psync: dict, source: dict):
    flag = 1;
    source['od1'] = 1E+99;
    p = -source['pow1']
    ...
    for i in range(0, psync['numbins']):
        if (psync['j'][i] > 0):
            ...
            """
            psync['a'][i] = kappa_nu = (
                        20.9 * kappa * (source['B'] ** ((p + 2.) / 2.)) * (5.67E+9 ** p) * b_factor(p) * (
                            psync['e'][i] ** (-(p + 4.) / 2.)))
...
"""
Calculate index of histogramm, giving adjacent bin-numbers num1, num2 and rest 
(value between 0 and 1 that gives the distance between num1 and num2)
"""
...
def get_flux(ple: dict, frequency: float):
    rest = 0.0
...
def get_flux2(ple: dict, nu: float, ichoice: int):
    ...
    if ((ichoice == 1) or (ichoice == 3)):
        tau = (ple['tau_i'][count1] + ple['tau_e'][0][count1]) * (1. - rest) + (
                    ple['tau_i'][count2] + ple['tau_e'][0][count2]) * rest
        flux *= math.exp(-tau)
...
def fill_le(ple: dict,
    ...
    """
    Broken power law
        xindex1 = source['pow1'] = -p1
        xindex2 = source['pow2'] = -p2
    """
...
parser = argparse.ArgumentParser(description='Synchrotron Self-Compton model \n \n \n \n Usage:\n\"spectrum '
                                             '[z=0.034] [Gamma=50] [theta=3] [B=0.025E-4] [radius=1E+12] '
                                             '[w_p_soll=10.] [log Emin=10.6] [log Emax=13.5] [log Ebreak=12.5] '
                                             '[p1=2] [p2=3] [ECflag=1] [Mbh=1E+9] [Mdot=1.] [blobDist=100] '
                                             '[EBLModel = Franceschini] \n\n\n')
...
parser.add_argument('-p1', default=2, help=' differential spectral index of the electron spectrum Emin to Ebreak',
                    type=float)
parser.add_argument('-p2', default=3, help=' differential spectral index of the electron spectrum Ebreak to Emax',
                    type=float)
parser.add_argument('-ec', default=1, help='Include external compton? 1=Yes, 0=No')
...
parser.add_argument('-dis', default=600,
                    help='height of the emitting volume above the accretion disk in Schwarzchild radii', type=float)
parser.add_argument('-ebl', default='Franceschini', help='Choice of EBL model, Franceschini, Finke or Dominguez',
                    type=str)
args = parser.parse_args()

start_time = time.time()

# main1(0, 0.047, 2.7 ,2.7, 0.04, 1.4E+16, 0.8, 3.5, 13.5, 11.45, 2., 3., 1, 1E+9, 4., 600.,"Finke")
main1(
    float(args.z),  # 0.047
    float(args.g),  # 2.7
    float(args.the),  # 2.7
    float(args.B),  # 0.04
    float(args.r),  # 1.4E+16
    float(args.wp),  # 0.8
    float(args.min),  # 3.5
    float(args.max),  # 13.5
    float(args.ebreak),  # 11.45
    float(args.p1),  # 2.
    float(args.p2),  # 3.
    float(args.ec),  # 1
    float(args.mbh),  # 1E+9
