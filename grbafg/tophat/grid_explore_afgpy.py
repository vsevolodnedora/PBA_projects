...
if __name__ == '__main__':
    ...
    pars = {
        ...
        "p_fs": [2.2, 2.4, 2.6, 2.8],  # [2.05, 2.1, 2.2, 2.3, 2.4, 2.6, 2.8, 3.0],
        "eps_e_fs": [0.5, 0.1, 0.01, 0.001],  # [0.001, 0.005, 0.01, 0.05, 0.1, 0.2, 0.3, 0.5],
        "eps_b_fs": [0.5, 0.1, 0.01, 0.001],  # [0.001, 0.005, 0.01, 0.05, 0.1],
    }
    ...
    for combination in all_combinations:
        ...
        result.append({par:val for par,val in zip(pars.keys(),combination)})
    ...
    #     results = pool.map(afgpy, range(len(result)))
    ...
    with h5py.File(outdir+"Y_afgpy.h5","w") as f:
        f.create_dataset("Y", data=params, dtype=np.float32)
