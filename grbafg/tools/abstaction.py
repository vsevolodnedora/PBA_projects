    ...
    def get(self,il,key) -> np.ndarray:
        ...
        if (key in self.keys):
            ...
            return np.array(np.array(self.refdata[f"layer={il}"]["Gamma"])) * \
                pba.utils.get_beta(np.array(np.array(self.refdata[f"layer={il}"]["Gamma"])))
        else:
...
class Base():
    ...
    def run_a(self, pars : dict, struct : dict, opts : dict, opts_grb : dict) -> pba.interface.PyBlastAfterglow:
        # workdir = os.getcwd()+'/'
        parfilefname = self.default_parfile_fpath.split("/")[-1]
        # copy the main parfile into workdir
        if (self.default_parfile_fpath!=self.workingdir+parfilefname):
    ...
    def plot_dyn(self, ax, pba : pba.interface.PyBlastAfterglow, v_n_x : str, v_n_y : str, layers=(), plot_layer = {}):
        cmap = plot_layer["cmap"] if "cmap" in plot_layer.keys() else 'viridis'
        cmap = cm.get_cmap(cmap)
...
class CasesFS(Base):
    ...
    def plot_lcs_ref(self, ax, ref : RefDataLC, nlayers : int, layers = (),):
        ...
        for il in range(int(nlayers)):
            if (((len(layers) > 0) and (il in layers)) or (len(layers)==0)):
                # --- plot ref data
                x_arr = ref.times()
                ax.plot(x_arr, ref.get(il), ls='-.', color=cmap(norm(il)), zorder=-1)
    ...
    def paper_plot_compare_spreading(self, axes, pars : dict, opts_a:dict, struct : dict,
                                     ref_dyn_fname : str or None, ref_lc_fname : str or None,layers=()):
        # run the code for given pars
        ...
        # ---------------------------------------------
        opts_a["method_spread"] = "AFGPY"
        opts_a["method_limit_spread"] = "Gamma0Frac"
        opts_a["fname_dyn"] = "dyn_a_afgpy.h5"
        opts_a["fname_light_curve"] = "lc_a_afgpy.h5"
        opts_a["fname_light_curve_layers"] = "lc_layers_a_afgpy.h5"
        pba_a = self.run_a(struct=struct, pars=pars, opts={}, opts_grb=opts_a)
        # ---------------------------------------------
        ...
        opts_a["fname_dyn"] = "dyn_a_aa.h5"
        opts_a["fname_light_curve"] = "lc_a_aa.h5"
        opts_a["fname_light_curve_layers"] = "lc_layers_a_aa.h5"
        pba_a = self.run_a(struct=struct, pars=pars, opts={}, opts_grb=opts_a)
        # ---------------------------------------------
        ...
        opts_a["fname_dyn"] = "dyn_a_adi.h5"
        opts_a["fname_light_curve"] = "lc_a_adi.h5"
        opts_a["fname_light_curve_layers"] = "lc_layers_a_adi.h5"
        pba_a = self.run_a(struct=struct, pars=pars, opts={}, opts_grb=opts_a)
        # ---------------------------------------------
...
class CasesFSRS(Base):
    ...
    def paper_plot_compare_fsrs(self, axes, pars : dict, opts : dict, opts_grb : dict, struct : dict, layers : tuple = ()):
        # run the code for given pars
        ...
        pba_a = self.run_a(struct=struct, pars=pars, opts=opts, opts_grb=opts_)
        # ---------------------------------------------

        self.plot_lcs(ax=axes[0], pars=pars, pba=pba_a, layers = layers,
                      plot={"ls":'-', "color":"red", "label":r"FS"},
        ...
        self.plot_lcs(ax=axes[0], pars=pars, pba=pba_a, layers = layers,
                      plot={"ls":'-', "color":"blue", "label":"FS \& RS"},
                      plot_layer={"ls":'-.', "cmap":"Blues", "alpha":1, "vmin":-50, "vmax":60})
        self.plot_dyn(axes[1], pba=pba_a, v_n_x="tburst", v_n_y="mom", layers=layers,
                      plot_layer={"ls":':', "cmap":"Blues", "alpha":1, "vmin":-50, "vmax":60})
        self.plot_dyn(axes[2], pba=pba_a, v_n_x="tburst", v_n_y="theta", layers=layers,
                      plot_layer={"ls":':', "cmap":"Blues", "alpha":1, "vmin":-50, "vmax":60})
        ...
        axes[0].plot(t, Fnu, ls='-', color='gray', label='afterglowpy')
    ...
    def paper_plot_resolution_rs(self, ax, pars : dict, opts_a : dict, struct : dict,
                                 layers : tuple, resolutions_a : tuple[tuple]):
        # run the code for given pars
    ...
    def compare_grbs(self, axes, pars : dict, opts_a : dict, opts_a_grb : dict,
                     struct : dict, layers:tuple, setups : tuple[dict]):
        """
